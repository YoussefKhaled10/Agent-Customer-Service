...
class CategoryModel:
    ...
    def create(cls, session: Session, name: str, slug: str | None = None,
        ...
        category = Category(name=name, slug=normalized_slug,
                            ...
                            is_active=True)
        session.add(category); session.flush(); return category

    @classmethod
    def list_categories(cls, session: Session, active_only: bool = False) -> list[Category]:
        statement = select(Category)
        if active_only: statement = statement.where(Category.is_active.is_(True))
    ...
    def set_active_status(cls, session: Session, category_id: int, is_active: bool) -> Category:
        ...
        if category is None: raise ValueError("Category was not found.")
        category.is_active = is_active; session.flush(); return category
