...
class PharmacistRequestModel:
    ...
    def list(cls, session: Session, status: PharmacistRequestStatus | None = None) -> list[PharmacistRequest]:
        ...
        if status: statement = statement.where(PharmacistRequest.status == status)
    ...
    def update_status(cls, session: Session, request_id: int,
        ...
        if request is None: raise ValueError("Pharmacist request was not found.")
        request.status = status
        if assigned_to is not None: request.assigned_to = assigned_to.strip() or None
